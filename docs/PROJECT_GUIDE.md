...
- 고객 그리드와 상세 정보는 우측 실행 패널처럼 독립된 패널 형태로 분리합니다.
- 기본 상하 비율은 65:35입니다.
- `--left-top-size` CSS 변수로 상단 고객 그리드 패널 높이를 관리합니다.
- 사용자는 좌측 상하 스플리터를 드래그하거나 위/아래 방향키로 비율을 조정할 수 있습니다.
...
- 고객 메뉴는 조회 결과 중 가장 최근 등록된 첫 번째 고객을 자동 선택합니다.
- 파이프라인, 견적, 계약 메뉴도 조회 결과의 첫 번째 행을 자동 선택합니다.
- 캘린더 메뉴는 기본적으로 오늘 날짜를 선택하고 오늘 첫 번째 일정을 상세 패널에 표시합니다.
- 에이전트 명령으로 영업활동 일정이 등록된 뒤 캘린더가 열리면 오늘 첫 일정이 아니라 방금 등록된 활동을 우선 선택합니다.
- 데이터가 없으면 상세 패널에는 조회된 데이터가 없다는 상태를 표시합니다.

하단 상세 패널 항목:

- 고객: 회사, 이름, 직무/직위, 휴대전화, 이메일, 주소, 홈페이지
- 파이프라인: 영업기회명, 회사명, 고객명, 상태, 단계, 확률, 금액, 종료예정일
- 캘린더: 회사명, 고객명, 활동 유형, 활동 내용, 활동 상태, 예정 일시, 완료 일시
- 견적: 견적번호, 견적일, 견적명, 회사명, 고객명, 상태, 금액
- 계약: 계약번호, 계약명, 회사명, 고객명, 상태, 금액, 시작일, 종료일
