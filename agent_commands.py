...
LIST_QUERY_TARGET_RE = re.compile(r"(고객|파이프라인|영업기회|캘린더|영업활동|일정|견적|계약)")
LIST_QUERY_ACTION_RE = re.compile(r"(리스트|목록|조회|찾아|검색|보여|알려)")
...
def matches_business_record_list_query(message: str) -> bool:
    text = message or ""
    return bool(LIST_QUERY_TARGET_RE.search(text) and LIST_QUERY_ACTION_RE.search(text))
...
AGENT_COMMAND_CASES: tuple[AgentCommandCase, ...] = (
    AgentCommandCase(
        case_id="business_record_list_query",
        title="업무 화면 리스트 DB 조회",
        category="list_query",
        priority=10,
        handler_name="handle_business_record_list_query",
        description="고객, 파이프라인, 캘린더/영업활동, 견적, 계약 리스트 요청을 화면 컨텍스트가 아니라 DB 조회로 처리한다.",
        matcher=matches_business_record_list_query,
        requires_customer_preflight=False,
        audit_action="list",
        audit_entity_type="business_record",
        flow_steps=(
            "메시지에서 조회 대상 화면을 판별한다.",
            "직위, 상태, 회사명, 고객명 등 지원 가능한 조건을 추출한다.",
            "현재 세션의 tenant_id와 user_id 범위로 DB를 조회한다.",
            "조회 결과를 조건으로 필터링하고 상위 결과를 요약한다.",
            "프론트가 해당 메뉴를 열 수 있도록 target_menu를 함께 반환한다.",
        ),
        test_points=(
            "고객 리스트 요청이 일반 LLM이 아니라 DB 리스트 조회로 라우팅되는지 확인한다.",
            "파이프라인, 캘린더/영업활동, 견적, 계약 리스트 표현이 같은 케이스로 라우팅되는지 확인한다.",
            "조건이 포함된 조회가 화면 컨텍스트가 아닌 DB 결과를 기준으로 답변하는지 확인한다.",
        ),
    ),
    AgentCommandCase(
